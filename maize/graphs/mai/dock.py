...
class GlideDocking(Graph):
    """Dock a molecule in the form of a SMILES string or IsomerCollection to a protein using Glide/LigPrep"""
    ...
    def build(self) -> None:
        ...
        ligand_loader = self.add(LoadMolecule)
        ligand_loader.path.optional = True
        ...
        self.map_port(ligand_loader.path, name="reference_ligand_file")
...
def dock_glide() -> Workflow:
    ...
    flow.connect_all((load.out, glide.inp), (glide.out, save.inp))
...
def dock_multi(n_grids: int = 2) -> Workflow:
    ...
    void = flow.add(Void)
    comb = flow.add(CombineScoredMolecules)
