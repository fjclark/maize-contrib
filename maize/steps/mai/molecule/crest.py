...
def create_constraints_crest(isomer: Isomer, path: Path) -> str:
    ...
    constraint_file = "{}/{}_crest_constraints.inp".format(path, isomer.name)
        ...
        with open(xyz, "r") as f:
            ...
            for line in f.readlines():
                ...
                if read_coords and line_lst[0] in atom_string:
                    ...
                    log.info(f"this is num atoms: {number_atoms}")
                    atom_id = 1
                    read_coords = False
                    read_energy = True
                    xyz_str_single = line
    ...
    def _parse_crest_outputs(
        ...
        for i, mol_folder in enumerate(mol_outputs):
            for j, iso_dirname in enumerate(mol_folder):
                ...
                with open(iso_dirname / "crest_out.txt", "w") as out:
                ...
                # Set isomer tag with relative exit code.
    ...
    def run(self) -> None:
        ...
        for i, mol in enumerate(mols):
            ...
            for j, isomer in enumerate(mol.molecules):
                ...
                input_flname = "input.sdf"

                # check for constraints to apply to the calculations
                if isomer.has_tag("constraints"):
                    constraints = "--cinp " + create_constraints_crest(isomer, iso_path)
                ...
                try:
                    isomer.to_sdf(path=iso_path / input_flname)
                except ValueError as err:
                ...
                if isomer.has_tag("constraints"):
                    keywords += f" --noreftopo {constraints}"
                command = f"{self.runnable['crest']} {input_flname} {keywords}"
        ...
        results = self.run_multi(
            ...
            batch_options=JobResourceConfig(
                cores_per_process=self.n_threads_job.value,
                custom_attributes={"mem": "2G"},
                exclusive_use=False,
                walltime="02:00:00",
            ),
