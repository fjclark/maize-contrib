            ...
            if mode == "minimum":
                if not len(self.gaussian_heavy_atoms()) == 0:
                    route_section = (
                        ...
                        + solvent
                        + ",dovacuum) "
                        + self.functional("wb97xd/genecp", oniom)
...
class Loader:
    ...
    * RDKIT MOL to Structure
    ...
    @staticmethod
    def molecule_from_isomer(conf: Conformer, path: Path, tag_name: str) -> "Structure":
        """
        Generate a Structure object from the conformer object.
        ...
        ----------
        path
            Conformer xyz file path
        tag_name
            Name assigned to the conformer/isomer object as tag
        ...
        try:
            label = str(conf.tags[tag_name])
            print(f"tag {label} for {path}")
        except KeyError:
            label = "tag undefined"
            print(f"no tag for {path}")
