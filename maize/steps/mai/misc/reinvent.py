...
class Mol2MolStandalone(Node):
    ...
    def run(self) -> None:
        ...
        scoring_params = {
            "mw_low": mw - 75,
            "mw_high": mw + 75,
            "clogp_low": round(clogp - 1.5, 2),
            "clogp_high": round(clogp + 1.5, 2),
            "hbd_low": hbd,
            "hbd_high": hbd + 2,
            "halo_low": halos,
            "halo_high": halos + 5,
            "hetero_low": heteros,
            "hetero_high": heteros + 7,
        }
        ...
        if docking_reference_score:
            ...
            # set scoring conf
            scoring_params["docking_low"] = docking_reference_score - 3
            scoring_params["docking_high"] = docking_reference_score + 3
        ...
        readlog = read_log(self.work_dir / REINVENT_LOGFILE)
        ...
        while worker.is_alive():
            self.logger.debug(f"Checking if ReInvent is still running {count}")
