...
class PrepareGrid(Node):
    ...
    def run(self) -> None:
        ...
        if self.inp_ligand.ready():
            ...
            self.logger.info("Using reference ligand '%s'", lig.to_smiles(remove_h=True))
        ...
        with batch_file.open("w") as file:
            ...
            for mol in mols:
                for isomer in mol.molecules:
                    ...
                    # Create pdbqt input
                    _mol2pdbqt(
                        ligand, isomer, constraints=constraints if self.constraints.value else None
                    )
...
class VinaFlex(_Vina):
    ...
    def run(self) -> None:
        ...
        # Convert each pose to SDF, update isomer conformation
        res = self._parse_flex_adv_outputs(mols, mol_outputs, receptor=self.receptor.filepath)
        self.out.send(mols)
