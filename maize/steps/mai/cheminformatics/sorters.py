...
class TagSorter(Node):
    ...
    def _matcher(self, mol: Isomer, tokens: list[str]) -> bool:
        ...
        match tokens:
            ...
            case _:
                self.logger.warning(
                    "Invalid predicate '%s' for isomer '%s'", " ".join(tokens), mol.inchi
                )
                return False
    ...
    def run(self) -> None:
        ...
        for line, out in zip(self.sorter.value, self.out):
...
class TestSuiteSorter:
    def test_TagSorter(self, test_config: Config) -> None:
        ...
        result = rig.setup_run(inputs={"inp": [mols]}, parameters={"sorter": sorter}, n_outputs=3)
